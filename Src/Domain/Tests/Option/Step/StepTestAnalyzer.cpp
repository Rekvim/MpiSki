...
// Запуск нового шага
void StepTestAnalyzer::startStep(double from, double to, quint64 time)
{
    m_state = StepState();
    m_state.from = from;
    m_state.to = to;
...
void StepTestAnalyzer::updateStep(double pos, quint64 time)
{
...
void StepTestAnalyzer::finishStep()
{
    if (!m_hasStep) return;
