    ...
    )
{
    ...
    report.data.push_back({"Отчет ЦТ", 5, 14, valveInfo.positionNumber});
    report.data.push_back({"Отчет ЦТ", 6, 14, valveInfo.serialNumber});
    report.data.push_back({"Отчет ЦТ", 7, 14, valveInfo.valveModel});
    report.data.push_back({"Отчет ЦТ", 8, 14, valveInfo.manufacturer});
    report.data.push_back({"Отчет ЦТ", 9, 14, valveInfo.DN + "/" + valveInfo.PN});
    report.data.push_back({"Отчет ЦТ", 10, 14, valveInfo.positionerModel});
    // report.data.push_back({"Отчет ЦТ", 11, 14, ???}); // Соленоидный клапан
    // report.data.push_back({"Отчет ЦТ", 12, 14, ???}); // Концевой выключатель/датчик положения
    report.data.push_back({"Отчет ЦТ", 13, 14, safeToString(telemetry.supplyPressure)});
    report.data.push_back({"Отчет ЦТ", 14, 14, otherParams.safePosition});
    report.data.push_back({"Отчет ЦТ", 15, 14, valveInfo.driveModel});
    report.data.push_back({"Отчет ЦТ", 16, 14, otherParams.strokeMovement});
    report.data.push_back({"Отчет ЦТ", 17, 14, valveInfo.materialStuffingBoxSeal});
