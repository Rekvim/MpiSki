    ...
    )
{
    ...
    report.data.push_back({"Отчет ЦТ",11, 13, QString::asprintf("%.2f", telemetryStore.supplyRecord.pressure_bar)});
    ...
    report.data.push_back({"Отчет ЦТ", 28, 8, QTime(0,0).addSecs(telemetryStore.cyclicTestRecord.totalTimeSec).toString("mm:ss.zzz")});
