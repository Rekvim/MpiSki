    ...
    )
{
    ...
    report.data.push_back({sheet_1, 84, 8, telemetryStore.cyclicTestRecord.sequence});
    report.data.push_back({sheet_1, 86, 8, QString::number(telemetryStore.cyclicTestRecord.cycles)});
    report.data.push_back({sheet_1, 88, 8, QTime(0,0).addSecs(telemetryStore.cyclicTestRecord.totalTimeSec).toString("mm:ss.zzz")});
